...
### Архитектура Данных (Связи)

#### 1. Пользователь (`User`) <-> Квест (`Quest`)
-   **Связь**: Один ко многим (`One-to-Many`).
-   **Описание**: Один `Пользователь` может создать **много** `Квестов`, но каждый `Квест` принадлежит только **одному** `Пользователю` (создателю).
-   **Реализация**: В схеме `Quest` есть поле `creator`, ссылающееся на `User`. В схеме `User` есть массив `quests` для хранения ID созданных квестов.

#### 2. Пользователь (`User`) <-> Монстр (`Monster`)
-   **Связь**: Один к одному (`One-to-One`).
-   **Описание**: У каждого `Пользователя` есть только **один** уникальный `Монстр`.
-   **Реализация**: В схеме `Monster` есть поле `user` с уникальным индексом, которое ссылается на `User`.

#### 3. Квест (`Quest`) <-> Решение (`Submission`)
-   **Связь**: Один ко многим (`One-to-Many`).
-   **Описание**: Один `Квест` может иметь **много** `Решений` от разных пользователей, но каждое `Решение` относится только к **одному** `Квесту`.
-   **Реализация**: В схеме `Submission` есть поле `quest`. В схеме `Quest` есть массив `submissions`.

#### 4. Пользователь (`User`) <-> Решение (`Submission`)
-   **Связь**: Один ко многим (`One-to-Many`).
-   **Описание**: Один `Пользователь` может отправить **много** `Решений`, но каждое `Решение` имеет только **одного** автора.
-   **Реализация**: В схеме `Submission` есть поле `author`, которое ссылается на `User`.

#### 5. Пользователь (`User`) <-> Таблица Лидеров (`Leaderboard`)
-   **Связь**: Один к одному (`One-to-One`).
-   **Описание**: Для каждого `Пользователя` создается **одна** запись в таблице лидеров для хранения его очков.
-   **Реализация**: В схеме `Leaderboard` есть поле `user`, ссылающееся на `User`.
